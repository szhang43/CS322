...
def check():
 ...
 # Respond appropriately
    rslt = {} #empty dictionary 
    if matched and in_jumble and not (text in matches): # if it is a valid match
        # Cool, they found a new word
        rslt["matches"] = "match" # add match to matches in the rslt dicitonary
        matches.append(text) # appended the new found word to matches
        flask.session['matches'] = matches # use flask.session to update list
        if len(matches) >= flask.session['target_count']: ## checks the count of the list is target 
            rslt["success"] = "success" # if so, add success into the rslt dicionary
        return flask.jsonify(result = rslt) # return result if satisfied
    elif not matched: 
        rslt["not_found"] = "not_found"
    elif text in matches:
        rslt["already_found"] = "already_found"
    elif not in_jumble: 
        rslt["not_jumble"] = "not_jumble"
