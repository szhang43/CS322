...
def respond(sock):
    ...
    if len(parts) > 1 and parts[0] == "GET":
        ...
        if re.search(r'\.\.', parts[1]) or re.search(r'~', parts[1]): #Regex check for invalid character ".." and "~"
            transmit(STATUS_FORBIDDEN, sock) # Transmit forbidden message
            transmit("\nForbidden Character in filename:{}\n".format(parts[1]), sock) # Print out the forbidden message with more info

        elif os.path.exists(path+parts[1]): # Check if the file exits, here checks if it does
            f = open(path+parts[1], "r") # open the file
            content = f.read() # reads the file
            transmit(STATUS_OK, sock) # transmit the file is okay message
            transmit(content, sock)   # displayes the contents of the file

        elif os.path.exists(path+parts[1]) == False: # if file does not exist
            transmit(STATUS_NOT_FOUND, sock) # transmit message file not found
            transmit("\nFile not found: {}\n".format(parts[1]), sock) # display message of file not found error
    else:
